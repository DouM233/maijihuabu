...
function GridCropNode(props: NodeProps) {
  const { data, selected, id } = props;
  const nodeData = data as GridCropNodeData;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border">
        <div className="w-6 h-6 rounded-md bg-pink-100 text-pink-600 dark:bg-pink-900/30 dark:text-pink-400 flex items-center justify-center">
          <Grid3x3 className="w-3.5 h-3.5" />
        </div>
        <span className="text-sm font-medium text-foreground">网格裁切</span>
      </div>
